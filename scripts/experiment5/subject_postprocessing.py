...
def main():
    for subject in range(5, 6):
        ...
        to_base_frame(path)
        # Clip the demos temporally, essential for getting an usable alignment afterwards
        fig, ax = plt.subplots(4, 4, figsize=(16, 8))
        processed = load_datasets(path)
        for idx, dataset in enumerate(processed):
            ...
            clip_datasets(path, float(lower_t), float(upper_t), idx, True)
            for i, j in product(range(4), range(4)):
                ax[i,j].clear()
            dataset = load_datasets(path, idx)
            plot_demo(ax, dataset)
            plt.show(block=False)
            input("Press any key to continue")

        # Align w soft-DTW
        align_datasets(path, gamma=2.5)
        # Correct the direction of the force
        flip_fz(path)
        # Just in case Motive screwed something up
