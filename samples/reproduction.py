...
if True or not exists(join(ROOT, 'controller/KP_pos_gains.npy')):
...
if not exists(join(ROOT, 'controller/KP_rot_gains.npy')):
    ...
    KD_rot = np.load(join(ROOT, 'controller/KD_rot_gains.npy'))
...
for i in range(poses.shape[-1]):
    ...
    dxe1 = (KD_pos@(KP_pos@xf1 - np.array(current_speed[:3]).reshape(-1,1))).flatten()
    dxe2 = (KD_rot@(KP_rot@xf2 - np.array(current_speed[3:]).reshape(-1,1))).flatten()
